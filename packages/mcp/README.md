...
| `feedock_create_milestone`, `feedock_update_milestone`               | `confirm:true` when the milestone is or becomes PUBLIC — its title, status, and live progress show on your public roadmap. Update reads the milestone's current visibility, so editing one that is already public is gated too. Owner/Admin.                                                                                                                                           |
| `feedock_update_roadmap_item`                                        | `confirm:true` when the item is or becomes PUBLIC — editing live public copy is a public write. Reads the item's current visibility. Owner/Admin.                                                                                                                                                                                                                                      |
